...
<body>
    <main class="container">
        ...
        <div class="readmesite-footer">
            <p>Generated by <a href="https://readme.site" target="_blank" rel="noopener noreferrer">READMEsite</a>{{FOOTER_REPO_SOURCE}}</p>
        </div>
    </main>
</body>
</html>`;
...
async function main() {
    ...
    try {
        ...
        if (localMode) {
            ...
            let linkUrl = '';
            let linkText = actualSourceRepoInfo.repoName;
            if (actualSourceRepoInfo.type === 'github') { linkUrl = `https://github.com/${actualSourceRepoInfo.user}/${actualSourceRepoInfo.repoName}`; linkText = `${actualSourceRepoInfo.user}/${actualSourceRepoInfo.repoName}`; }
            else if (actualSourceRepoInfo.type === 'gitlab') { linkUrl = `https://gitlab.com/${actualSourceRepoInfo.userOrGroupPath}/${actualSourceRepoInfo.repoName}`; linkText = `${actualSourceRepoInfo.userOrGroupPath}/${actualSourceRepoInfo.repoName}`; }
            else if (actualSourceRepoInfo.type === 'unknown_gitlab_like') { linkUrl = `https://${actualSourceRepoInfo.host}/${actualSourceRepoInfo.userOrGroupPath}/${actualSourceRepoInfo.repoName}`; linkText = `${actualSourceRepoInfo.userOrGroupPath}/${actualSourceRepoInfo.repoName}@${actualSourceRepoInfo.host}`; }
            
            if (linkUrl) {
                footerRepoSourceHtml = ` from <a href="${linkUrl}" target="_blank" rel="noopener noreferrer">${linkText}</a>`;
                if (!ogUrlToUse) ogUrlToUse = linkUrl; 
            } else if (actualSourceRepoInfo.originalIdentifier) {
                footerRepoSourceHtml = ` from ${actualSourceRepoInfo.originalIdentifier}`;
            }
        }

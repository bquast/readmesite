...
<body>
    ...
    <footer>
        <p>Generated by <a href="https://github.com/bquast/readmesite" target="_blank" rel="noopener noreferrer">READMEsite</a></p>
    </footer>
</body>
</html>`;
...
async function parseRepoIdentifier(identifier) {
    ...
    // Attempt 1: Treat as a full URL
    try {
        const url = new URL(identifier); // Requires a full URL like https://...
        const hostname = url.hostname.toLowerCase();
        const pathParts = url.pathname.split('/').filter(Boolean);

        if (pathParts.length < 2) { // Need at least user/group and repo name
        ...
        if (hostname === 'github.com') {
            ...
            // For other HTTPS URLs, assume a GitLab-like structure for raw access (heuristic).
            const repoName = pathParts[pathParts.length - 1];
            const userOrGroupPath = pathParts.slice(0, pathParts.length - 1).join('/');
        ...
        // Not a valid full URL or parsing failed (e.g. URL path segments error).
        // Swallow error and proceed to treat as 'user/repo' shorthand if applicable.
        // If 'e' was from new URL(identifier) itself, it means 'identifier' is not a URL.
        if (!(e instanceof TypeError && e.message.includes("Invalid URL"))) { // Log other URL parsing errors
             console.warn(`URL parsing attempt failed for '${identifier}': ${e.message}. Will try as 'user/repo'.`);
        }
    }

    // Attempt 2: Treat as 'user/repo' shorthand
    // A simple 'user/repo' should not contain '://' or be a partial http/https
    ...
    if (parts.length === 2 && parts[0] && parts[1] && !parts[0].includes('.') && !parts[1].includes('.')) { // Basic check
...
async function tryFetchForProvider(providerRepoInfo, branches, filePath) {
    ...
    for (const branch of branches) {
        ...
        if (providerRepoInfo.type === 'github') {
            ...
            // This case should ideally not be reached due to parseRepoIdentifier logic
        ...
        try {
            ...
            if (response.status === 404) {
                ...
                console.warn(lastBranchError.message); // Log and continue to next branch
            } else {
                // For other HTTP errors (not 404), this is a more significant issue for this provider/repo.
                // We'll stop trying other branches for this provider.
                throw new Error(`Failed to fetch ${filePath} from ${readmeUrl}. Status: ${response.status} ${response.statusText}`);
            }
        } catch (fetchError) { // Catches network errors or errors thrown from non-404 HTTP responses
            lastBranchError = new Error(`Network/fetch error for ${readmeUrl}: ${fetchError.message}`);
            console.warn(lastBranchError.message);
            // If one URL fetch has a network error, subsequent branches for same repo might too.
            // For now, we stop trying further branches for this provider on a network/fetch error.
            return { content: null, error: lastBranchError }; 
        }
    }
    // If loop completes, all branches for this provider failed for this filePath.
...
async function fetchReadmeContent(repoInfo) {
    ...
    if (repoInfo.type === 'user_slash_repo') {
        ...
        // It's a specific provider type from a full URL (github, gitlab, unknown_gitlab_like)
...
async function generateSite(readmeContent, pageTitleBase, outputDir) {
    // ... (generateSite remains the same as previous version)
...
async function main() {
    ...
    try {
        ...
        await generateSite(readmeContent, repoInfo.repoName, path.resolve(outputDirArg));

        console.log("\n✅ Site generation complete!");
        console.log(`\nStatic site files are located in: ${path.resolve(outputDirArg)}`);
        // ... (rest of deployment instructions remain the same)
